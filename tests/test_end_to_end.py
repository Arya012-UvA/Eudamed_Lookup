...
def test_ui_says_filters_match_exactly(ui_server_fresh):
    """A near-miss returns nothing, so the page must say so or it reads as
    "this device is not registered"."""
    assert ui_server_fresh.json("/api/health")["exact_match_filters"] is True
    page = ui_server_fresh.text("/")
    assert "matches names exactly" in page
    assert "does <em>not</em> mean the device is" in page

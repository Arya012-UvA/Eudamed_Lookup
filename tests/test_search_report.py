...
# --- cache ---------------------------------------------------------------
def test_cache_dedupes_by_identifier():
    from eudamed.cache import RowCache
    cache = RowCache()
    cache.add([{"UUID": "a", "TRADE_NAME": "X"}, {"UUID": "a", "TRADE_NAME": "X"},
               {"PRIMARY_DI": "b"}, {"BASIC_UDI": "c"}])
    assert len(cache) == 3


def test_cache_records_truncated_partitions():
    from eudamed.cache import RowCache
    cache = RowCache()
    cache.add([{"UUID": str(n)} for n in range(1000)],
              source={"RISK_CLASS_ID": 1}, truncated=True)
    cache.add([{"UUID": "x"}], source={"RISK_CLASS_ID": 2}, truncated=False)
    assert len(cache.truncated_partitions) == 1
    assert cache.truncated_partitions[0]["filter"] == {"RISK_CLASS_ID": 1}


def test_cache_round_trip(tmp_path):
    from eudamed.cache import RowCache
    cache = RowCache()
    cache.add([{"UUID": "a", "TRADE_NAME": "MindDoc"}], source={"x": 1}, truncated=True)
    path, _meta = cache.save(str(tmp_path / "c" / "udi.jsonl"))
    loaded = RowCache.load(path)
    assert len(loaded) == 1
    assert next(loaded.devices()).trade_name == "MindDoc"
    assert loaded.truncated_partitions          # preserved via the meta file


def test_match_cached_scores_locally():
    """Approximate names are matchable only once rows are held locally."""
    from eudamed.cache import RowCache
    from eudamed.search import match_cached
    cache = RowCache()
    cache.add([{"TRADE_NAME": "MindDoc", "MF_NAME": "MindDoc Health GmbH",
                "MF_SRN": "DE-MF-1", "UUID": "u1"}])
    result = match_cached(list(cache.devices()), Target("Mind Doc", country="DE",
                                                        keys=["Mind Doc"]))
    assert result["status"] == "found"
    assert result["candidates"][0]["trade_name"] == "MindDoc"


def test_match_cached_still_caps_manufacturer_evidence():
    from eudamed.cache import RowCache
    from eudamed.search import match_cached
    cache = RowCache()
    cache.add([{"TRADE_NAME": "Moodpath", "MF_NAME": "MindDoc Health GmbH",
                "MF_SRN": "DE-MF-1", "UUID": "u2"}])
    result = match_cached(list(cache.devices()), Target("MindDoc", country="DE",
                                                        keys=["MindDoc"]))
    assert result["status"] == "not found"
    assert result["candidates"][0]["matched_on"] == "manufacturer"
